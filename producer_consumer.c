 ...
 *                              BUG1
                    GLOBAL DECLARATIONS UNINITIALIZED
                        CHECK BELOW FOR TAG BUG!
 
 *----------------------------------------------------------------------*/
long g_num_prod=0;//INITILAIZED /* number of producer threads */
...
int main(int argc, char **argv) {
  ...
  pthread_mutex_init(&queue.lock, NULL);//initialized now. BUG1.
  ...
  printf("Producer thread started with thread id %lu\n", producer_thread);
     ...
     *                              BUG2
            PTHREAD_JOIN CANNOT WORK IF THE THREAD IS DETACHED.
            FIX BY NOT DETACHING OR BY NOT JOINING THE THREAD.
                    I HAVE COMMENTED OUT THE DETACHING.
...
void *producer_routine(void *arg) {
  ...
  /* Decrement the number of producer threads running, then return */
     ...
     *                              BUG3
            G_NUM_PROD IS SHARED. PROTECT IT BY LOCKING AND
                    UNLOCKING BEFORE AND AFTER USE
     
     *----------------------------------------------------------------------*/
    
    pthread_mutex_lock(&g_num_prod_lock);//FIX FOR BUG3. LOCK IT.
  --g_num_prod;
    pthread_mutex_unlock(&g_num_prod_lock);//FIX FOR BUG3. UNLOCK IT.
...
void *consumer_routine(void *arg) {
  ...
  //long count = 0; /* number of nodes this thread printed */
     ...
     *                              BUG4
                        DECLARE COUNT AS AN ACCESSIBLE POINTER
     
     *----------------------------------------------------------------------*/
 
    long* count = (long*)malloc(sizeof(long));//FIX FOR BUG4
    *count = 0;
  ...
  while(queue_p->front != NULL || g_num_prod > 0)
  {
    ...
    if (queue_p->front != NULL) {
       ...
       *                                BUG5
                                LOCK IT FOR EVERY LOOP!
       
       *----------------------------------------------------------------------*/
      pthread_mutex_lock(&queue_p->lock);//FIX FOR BUG5
      pthread_mutex_lock(&g_num_prod_lock);//FIX FOR BUG5
